...
router_google.get('/google/seed_old', (req, res) => {
...
router_google.post("/google-refresh-token", function (req, res) {
    ...
    async function run() {
        ...
        try {
            ...
            var auth0AccessToken = (await auth0_helpers.userIdAsync(accessToken)).split("|")[1]
        } catch(e) {
            return "error occurred"
        }
        ...
        try {
            ...
            var gRefresh = await auth0_helpers.userInfoRefresh(auth0AccessToken, mgmtToken)
        } catch(e) {
            return "error occurred"
        }
        
        console.log("Retrieved google refresh token in google-refresh-token: " + gRefresh)
...
router_google.post('/api/google/calendar/events', (req, res) => {
    ...
    console.log('google refresh token in api/google/calendar/events: ' + googleRefreshToken)

    fs.readFile(path.join(__dirname, '..', 'helpers/oauth_google/credentials.json'), (err, content) => {
...
function createUpdateGcalDB(gCalMeetings, req) {
    return new Promise(function (resolve, reject) {
        ...
        auth0_helpers.userId(accessTokenCookie, function (result) {
            ...
            googleCalEventsDB.bulkCreate(gCalArr, {
